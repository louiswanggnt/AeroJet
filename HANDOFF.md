...
- ✅ **端對端部署驗證通過 2026-05-12**：完整 push → Actions → FTP → auvita.tw 流程驗收 OK
  - GitHub Actions deploy-on-push 全綠（最後 run 25721522949）
  - 14 個線上 route 全 200：`/`、`/zh-TW`、`/zh-TW/`、`/en`、`/ja`、6 個 zh-TW 子頁面、`/llms.txt`、`/en/about`、`/ja/about`
  - SEO `<link rel="canonical" href="https://www.auvita.tw/zh-TW"/>` 正確（ADR-003 落地）
  - hreflang alternate 三語都指向 auvita.tw
  - Promotions 輪播當期活動「五月美肌煥新月」（2026-05-promo-1）顯示正確
  - llms.txt 100% 繁中、內容完整
- ✅ 修補：`public/.htaccess`（Apache routing：根重導 `/` → `/zh-TW`、`DirectorySlash Off`、`.html` 副檔名 fallback、trailing-slash 移除、404 / 快取 / gzip）
- ✅ 修補：workflow 在 `npm ci` 前 `npm install -g npm@latest`（runner 預設 npm 10.x 無法解析本地 npm 11 產出的 lock）
- ✅ 修補：deploy-on-push 的 `paths` filter 由 `public/images/**` 改為 `public/**`（含 `.htaccess`）

## 13. 待處理 TODO

- [x] **GitHub Secrets 設定** — 已設於 `louiswanggnt/AeroJet`（FTP_HOST/PORT/USER/PASSWORD/REMOTE_PATH + NEXT_PUBLIC_SITE_URL）
- [x] **第一次部署驗證** — 2026-05-12 通過。FTP 預設增量模式即可，不需開 `dangerous-clean-slate`
...
> 已決定不做：①FTPS/SFTP 升級（維持 FTP）②repo 轉組織帳號 ③臨床案例日期改用資料層（不再顯示日期）。
